    ...
    rtsp_nodes = [
        Node(
            ...
            parameters=[{
                ...
                'max_queue_size': 1,  # Set max queue size to 1 for minimal latency
                "use_sim_time": False
            }],
        ...
        Node(
            ...
            name='front_camera',
            parameters=[{
                'rtsp_url': 'rtsp://192.168.2.50:554/stream',
            ...
            remappings=[
                # Disable all possible image transport plugins
                ('/front_camera/image_raw/compressed', '/front_camera/disabled_compressed'),
                ('/front_camera/image_raw/compressedDepth', '/front_camera/disabled_compressedDepth'),
                ('/front_camera/image_raw/theora', '/front_camera/disabled_theora'),
            ],
        ),
        Node(
            package='multicamera_pipeline',
            ...
            name='fork_camera',
            parameters=[{
                'rtsp_url': 'rtsp://192.168.2.45:554/stream',
                'topic_name': 'fork_camera/image_raw',
                'frame_rate': 15.0,  # Reduced frame rate for reliability
                'image_width': 640,
                'image_height': 480,
                'pipeline_type': 4,  # Use UDP pipeline for lower latency
                'silent_mode': silent_mode,
                'connection_retry_interval': 5,  # Wait 5 seconds between retries
                'max_connection_attempts': 20,  # Maximum retry attempts
                'max_queue_size': 1,  # Set max queue size to 1 for minimal latency
            ...
            remappings=[
                # Disable all possible image transport plugins
                ('/fork_camera/image_raw/compressed', '/fork_camera/disabled_compressed'),
                ('/fork_camera/image_raw/compressedDepth', '/fork_camera/disabled_compressedDepth'),
                ('/fork_camera/image_raw/theora', '/fork_camera/disabled_theora'),
            ],
        ),
    ]
...
def generate_launch_description():
    # Set GStreamer debug environment variables - disable debug output
    gst_debug = SetEnvironmentVariable(
        name='GST_DEBUG',
        value='0'  # Disable all debug output
    )
    
    # Declare launch arguments
    silent_arg = DeclareLaunchArgument(
        'silent_mode',
        default_value='true',  # Set to true for minimal output
